...
class MenuComponent {
    ...
    private _isItemSubShown = (item: HTMLElement) => {
        ...
        if (sub) {
            if (this._getItemSubType(item) === 'dropdown') {
                ...
                return (
                    subHTMLElement.classList.contains('show') && subHTMLElement.hasAttribute('data-popper-placement')
                );
            } else {
    ...
    public static hideDropdowns = (skip: HTMLElement | undefined) => {
        const items = document.querySelectorAll<HTMLElement>('.show.menu-dropdown[data-kt-menu-trigger]');
