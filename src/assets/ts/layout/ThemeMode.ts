...
class ThemeMode {
    ...
    private handleMenu = (): void => {
        this.menu?.querySelectorAll<HTMLElement>('[data-kt-element="mode"]')?.forEach((item: HTMLElement) => {
    ...
    public swapStylesheet = () => {
        const primeStyleLink = document.getElementById('primeTheme');
        let styleLink: string;
        if (this.getMode() === 'dark') {
            styleLink = 'prime-dark-theme.css';
        } else {
            styleLink = 'prime-light-theme.css';
        }
        primeStyleLink?.setAttribute('href', styleLink);
    };
